 ...
 * 
 * <p>
 * Multiple aliases exist for the various units of time (ie: <code>MINUTE</code>
 * and <code>MINUTES</code>; <code>MILLI</code>, <code>MILLIS</code>,
 * <code>MILLISECOND</code>, and <code>MILLISECONDS</code>.) The complete list
 * can be found by inspecting the keySet of <code>CALENDAR_UNITS</code>.
 * </p>
 * 
 * Originally <code>org.apache.solr.util.DateMathParser</code>
 */
public class DateMathParser {
	...
	public static void round(Calendar c, String unit) {
		...
		case Calendar.SECOND:
			...
			break;
		default:
			throw new IllegalStateException("No logic for rounding value (" + u
					+ ") " + unit);
		}
	...
	/**
	 * @param tz
	 *            The TimeZone used for rounding (to determine when hours/days
	 *            begin)
	 * @param l
	 *            The Locale used for rounding (to determine when weeks begin)
	 ...
	 *                positions in ParseExceptions are token positions, not
	 *                character positions.
	 */
	public Date parseMath(String math) throws ParseException {
		...
		while (pos < ops.length) {
			...
			if (1 != ops[pos].length()) {
				throw new ParseException("Multi character command found: \""
						+ ops[pos] + "\"", pos);
			}
			...
			case '/':
				if (ops.length < pos + 1) {
					throw new ParseException("Need a unit after command: \""
							+ command + "\"", pos);
				}
				try {
					round(cal, ops[pos++]);
				} catch (IllegalArgumentException e) {
					throw new ParseException("Unit not recognized: \""
							+ ops[pos - 1] + "\"", pos - 1);
				}
				break;
			case '+': /* fall through */
			case '-':
				...
				try {
					...
					throw new ParseException("Not a Number: \"" + ops[pos - 1]
							+ "\"", pos - 1);
				}
				if ('-' == command) {
					val = 0 - val;
